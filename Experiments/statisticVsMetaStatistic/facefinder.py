...
# TODO I'm confused what the expected behavior when a node is a part of
...
def compute_rotation_system(graph):
    ...
    for v in graph.nodes():
        locations = [graph.nodes[w]["pos"] - graph.nodes[v]["pos"] for w in graph.neighbors(v)]
        neighbor_list = list(graph.neighbors(v))
        dictKey = dict(zip(neighbor_list, (-float(np.arctan2(x[1], x[0])) for x in locations)))
        neighbor_list.sort(key=dictKey.get)
...
def compute_face_data(graph):
    ...
    faces = set([cycle_around_face(graph, e) for e in graph.edges()]).union(set([cycle_around_face(graph, e[::-1]) for e in graph.edges()])) 
